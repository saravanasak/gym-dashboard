...
- Admins can deactivate users, preventing them from logging in and showing a message to contact the admin.
...
- Redesigned the plan management interface into a table for better readability, displaying plan details with edit and delete options.
...
- Added an admin notification system to alert about upcoming membership expirations within the current month.
- Notifications are displayed in a readable format, showing Member ID, Username, Plan ID, Last Payment details, and Expiry Date.
- Included a search feature to look up notifications by Member ID or Username for better user management.
- Enhanced the UI to ensure the notifications are displayed centrally and consistently, removing excess whitespace.

### 6. UI Enhancements
- Improved the UI for better readability and user interaction.
- Updated the Admin Dashboard layout to include a consistent side navigation with icons for easy navigation, and used a dark theme with yellow accents.
- Added search and filter functionality throughout the system to enhance navigation and ease of use.
...
We have implemented the core functionalities of the Admin, Staff, and Customer dashboards, each with role-specific access. Next, we will focus on testing, additional UI improvements, and refining the experience for each role.
